...
const createOrFetchPin = async (textQuery: string): Promise<string> => {
  try {
    ...
    const pinNameSearch = await supabase.from("pins").select("*").ilike("pin_name", textQuery); // Case-insensitive search
    ...
    const placeIdSearch = await supabase.from("pins").select("*").like("google_place_id", google_place_id);
